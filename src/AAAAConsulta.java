...
public class AAAAConsulta {

	static String AAAACons(ArrayList<String> ips2,String nombre,String ip_servidor,String ip_consultas1, String RRtype_consultas, ArrayList nombres,ArrayList ips,ArrayList<String> ttl) throws Exception{
		...
		AAAAResourceRecord nueva_ip;
		String ip_consultas,consulta;
		ip_consultas=ip_servidor;
	
		try{
				int i=0;
		Message question= new Message(nombre,RRType.AAAA,false);
	...
	*/
		...
		if((response.getAnswers()==null)||(response.getAnswers().isEmpty())){
			...
			if(response.getAdditonalRecords().isEmpty()||(response.getAdditonalRecords()==null)){
				...
				String r=nameserver.get(i).getRRType().toString();
				   if(r.equals("SOA")){;return "fin";}
				System.out.print("A"+" "+ip_consultas+" ");
				...
				return "fin";
					...
					//PrintStream ps= System.out;
					//Consultas.dump(System.out,nameserver.get(0));
					//RRType rtype=nameserver.get(0).getRRType();
					//System.out.print("No hay registro tipo " +rtype+" en seccion ADDITIONAL para ");
					
				//	if(rtype.equals("A")){
					//Consultas.dumpA(ps,(AResourceRecord)nameserver.get(0));
				//}else Consultas.dumpNS(ps,(NSResourceRecord)nameserver.get(0));
			...
			AResourceRecord nueva_ipx=Consultas.dumpA_address((AResourceRecord)additional.get(i));
			
		    String ip=nueva_ipx.getAddress().getHostAddress();
			...
			nueva_ip=Consultas.dumpAAAA_address((AAAAResourceRecord)answers.get(0));
